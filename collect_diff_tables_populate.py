...
class INC_Company_Mgmt(object):
    ...
    def read_inc_grids_doc_wise(self, cur, conn, db_name, doc_id_lst, company_id, cid_path, inc_project_id):
        ...
        rqry = """ SELECT doc_id, page_no, grid_id FROM table_mgmt WHERE doc_id in (%s); """%(doc_str)
        cur.execute(rqry)
        d_data = cur.fetchall()
        
        old_doc_info = {}
        for rd in d_data:
            d_c, pg, grd = map(str, rd)
            dpg_tab = '{0}_{1}_{2}'.format(d_c, pg, grd)
            json_data = self.read_json_file(dpg_tab, company_id)
            old_doc_info.setdefault(d_c, {})[dpg_tab] = json_data
        ...
        for row_data in res:
            ...
            new_dpg = '{0}_{1}_{2}'.format(docid, pageno, grid_id)
            new_doc_info.setdefault(str(docid), {})[new_dpg] = gdata_dct.get('data', {})
        ...
        self.check_diff_tables(old_doc_info, new_doc_info)
        return  
        
    def check_diff_tables(self, old_doc_info, new_doc_info):
        doc_wise_tab_diff = {}
        for nw_oc, nw_tab_dct in new_doc_info.iteritems():
            old_tab_dct = old_doc_info.get(nw_oc, {})
            if not old_tab_dct:continue
            new_tab_set = set(nw_tab_dct)
            old_tab_set = set(old_tab_dct)
            del_tables    = old_tab_set - new_tab_set
            added_tables  = new_tab_set - old_tab_set
            #print del_tables 
            #print added_tables
            if del_tables or added_tables:
                doc_wise_tab_diff.setdefault(nw_oc, {})
                if del_tables: 
                    doc_wise_tab_diff[nw_oc]['DEL'] = list(del_tables)
                if added_tables:
                    doc_wise_tab_diff[nw_oc]['ADD'] = list(added_tables)
                    
            intersect_tables = new_tab_set.intersection(old_tab_set)
            #print intersect_tables
            exist_changed_tables = {}
            for tab in intersect_tables:
                new_rc_dct = nw_tab_dct.get(tab, {})
                old_rc_dct = old_tab_dct.get(tab, {})
                new_rc_set = set(new_rc_dct)
                old_rc_set = set(old_rc_dct)
                rc_diff = new_rc_set - old_rc_set
                #print rc_diff
                if rc_diff: 
                    exist_changed_tables.setdefault(tab, []).append({'error_text':'NEW ROW-COL', 'rc':list(rc_diff)})
                else:
                    for old_rc, old_cell_dict in old_rc_dct.iteritems():
                        new_cell_dict = new_rc_dct.get(old_rc, {})
                        if not new_cell_dict:
                            exist_changed_tables.setdefault(tab, []).append({'error_text':"OLD-RC-DOESN'T EXISTS", 'rc':[old_rc]})
                            continue
                        new_txt      = new_cell_dict.get('data')
                        new_xml_ids  = new_cell_dict.get('xml_ids')
                        old_txt      = old_cell_dict.get('data')
                        old_xml_ids  = old_cell_dict.get('xml_ids') 
                        error_txt_lst = []
                        if new_txt != old_txt:
                            error_txt_lst.append('DIFFERENCE IN TEXT')
                        if new_xml_ids != old_xml_ids:
                            error_txt_lst.append('DIFFERENCE IN XML_IDS')
                        error_txt = '~'.join(error_txt_lst) 
                        if error_txt:
                            exist_changed_tables.setdefault(tab, []).append({'error_text':error_txt, 'rc':[old_rc]})
            if exist_changed_tables:
                doc_wise_tab_diff.setdefault(nw_oc, {})['EXISTS'] = exist_changed_tables
        
        insert_rows = []
        for doc, i_dct in doc_wise_tab_diff.iteritems():
            ad, dl, ex = i_dct.get('ADD', []), i_dct.get('DEL', []), i_dct.get('EXISTS', {})
            if ad or dl or ex:
                ad, dl, ex = map(json.dumps, (ad, dl, ex))
                insert_rows.append((doc, ad, dl, ex))
        
        if insert_rows:
            db_path = '/mnt/eMB_db/company_management/{0}/table_info.db'.format(company_id)
            print db_path
            conn, cur   = conn_obj.sqlite_connection(db_path)
            crt_stmt = """ CREATE TABLE IF NOT EXISTS doc_table_diff(row_id INTEGER PRIMARY KEY AUTOINCREMENT, company_id VARCHAR(50), added TEXT, deleted TEXT, existing TEXT); """      
            cur.execute(crt_stmt)
            for r in insert_rows:
                insert_stmt =           """ INSERT INTO doc_table_diff(company_id, added, deleted, existing) VALUES('%s', '%s', '%s', '%s'); """%r
...
if __name__ == '__main__':
    ...
    doc_id_str = sys.argv[2]
    doc_ids    = doc_id_str.split('#')

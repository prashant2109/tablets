...
import table_validation_store as tvs
ev_Obj = tvs.ErrorValidation()
...
class INC_Company_Mgmt(object):
    ...
    def update_table_boundry(self, grid_dct, pno, phcsv_i):
            ...
            if ddict[r_c]['ldr'] in ['value', 'hch', 'gh', 'vch']:
                        ...
                        for bx in bbox_lst:#xmin_ymin_xmax_ymax
                            if bx:
                                ...
                                bbox_d[pno]['w'][w]   = 1
                                bbox_d[pno]['h'][h]   = 1
                                if ddict[r_c]['ldr'] in ['value'] and ddict[r_c].get('data','').strip():
                                    bbox_gv_d.setdefault(int(pno), {'x':{}, 'y':{}, 'w':{}, 'h':{}})
                                    bbox_gv_d[pno]['x'][x]   = 1
                                    bbox_gv_d[pno]['y'][y]   = 1
                                    bbox_gv_d[pno]['w'][w]   = 1
                                    bbox_gv_d[pno]['h'][h]   = 1
        ...
        for k, v in bbox_gv_d.items():
            ...
            tmpbbox_d[k]    = [x, y, w,h]
        #print tmpbbox_d
        tmpbbox                         = tmpbbox_d.get(pno, [])
        grid_dct['table_boundry_gv']    = tmpbbox
        grid_dct['gv_bbox']             = tmpbbox_d
        return grid_dct 
    ...
    def read_inc_grids_doc_wise(self, cur, conn, db_name, doc_id_lst, company_id, cid_path, inc_project_id):
        ...
        for doc_id in doc_id_lst:
            ...
            if 1:#not os.path.exists(db_path):
                ...
                cmd1 = 'cp /var/www/html/WorkSpaceBuilder_DB/{0}/1/pdata/docs/{1}/dbdata/{1}_numeq.db /mnt/eMB_db/company_management/{0}/equality/ > /dev/null'.format(company_id, doc_id)
                try:
                    #os.system(cmd1)
                    #t = ''
                ...
                try:    
                    #os.system(cmd2)
                    process = subprocess.Popen(cmd2, stdout=f_null, stderr=subprocess.PIPE, shell=True)
                    error = process.communicate()
                except:pass                
                ...
                try:
                    #os.system(cmd3)
                    process = subprocess.Popen(cmd3, stdout=f_null, stderr=subprocess.PIPE, shell=True)
                    error = process.communicate()
                except:pass
                ...
                cmd = 'cp /var/www/html/WorkSpaceBuilder_DB/%s/1/pdata/docs/%s/dbdata/%s.db  /mnt/eMB_db/company_management/%s/equality/'%(inc_project_id, doc_id, doc_id, company_id)
                print cmd
                process = subprocess.Popen(cmd, stdout=f_null, stderr=subprocess.PIPE, shell=True)
                error = process.communicate()
                #os.system(cmd)
        ...
        try:
            self.index_doc_info(company_id, doc_id_lst)
        except:pass
        self.error_validation(company_id, doc_id_lst, doc_str)
        return  
        
    def error_validation(self, company_id, doc_id_lst, doc_str):
        for doc_id in doc_id_lst:
            ijson = {"company_id":company_id, "doc_id":doc_id}
            ev_Obj.table_validation(ijson, )
            ijson['doc_str'] = doc_str
            ev_Obj.return_table_exception_flg(ijson) 
        return 
        
    def index_doc_info(self, company_id, doc_id_lst):
        ijson = {'cmp_id':company_id, 'doc_ids':doc_id_lst}
        idx_Obj = ni.indexing()  
        idx_Obj.indexing(ijson)
        return
...
if __name__ == '__main__':
    ...
    #print ic_Obj.doc_group_info(company_id) ## D FUNC
